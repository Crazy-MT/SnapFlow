...
public enum SnapFlowKit {
	...
	private static func unregister(_ shortcut: Shortcut) {
		CarbonSnapFlowKit.unregister(shortcut)
		registeredShortcuts.remove(shortcut)
		activeShortcuts.remove(shortcut)
	}

	// TODO: Doc comment and make this public.
	static func unregisterAll() {
		CarbonSnapFlowKit.unregisterAll()
	...
	*/
	...
	private static func handleOnKeyDown(_ shortcut: Shortcut) {
		...
		guard !activeShortcuts.contains(shortcut) else {
			return
		}

		activeShortcuts.insert(shortcut)
	...
	static func simulateKeyDown(for shortcut: Shortcut) {
		handleOnKeyDown(shortcut)
	}

	static func simulateKeyUp(for shortcut: Shortcut) {
	...
	final class AppDelegate: NSObject, NSApplicationDelegate {
		func applicationDidFinishLaunching(_ notification: Notification) {
			SnapFlowKit.onKeyDown(for: .toggleUnicornMode) { [self] in
	...
	*/
	...
	final class AppDelegate: NSObject, NSApplicationDelegate {
		func applicationDidFinishLaunching(_ notification: Notification) {
			SnapFlowKit.onKeyUp(for: .toggleUnicornMode) { [self] in
				isUnicornMode.toggle()
			}
		}
	}
	...
	*/
	...
	private static let userDefaultsPrefix = "SnapFlowKit_"
...
extension Notification.Name {
	static let shortcutByNameDidChange = Self("SnapFlowKit_shortcutByNameDidChange")
}
